...
public class SwfTexture
{
    ...
    public static function fromFlipbook (lib :XflLibrary, movie :MovieMold, frame :int,
        ...
        const isJpg:Boolean =  movie.id.indexOf("$jpg") != -1;
        const isSingle:Boolean =  movie.id.indexOf("$single") != -1;
        const name :String = ns + movie.id + "_flipbook_" + frame;
        return new SwfTexture(name, clip, scale, quality, isJpg, isSingle);
    }
    ...
    public static function fromTexture (lib :XflLibrary, tex :XflTexture,
        ...
        const ns :String = useNamespace ? lib.location + "/" : "";
        const isJpg:Boolean =  tex.symbol.indexOf("$jpg") != -1;
        const isSingle:Boolean =  tex.symbol.indexOf("$single") != -1;
        const disp :DisplayObject = (instance is BitmapData) ?
            new Bitmap(BitmapData(instance)) : DisplayObject(instance);
        return new SwfTexture(ns + tex.symbol, disp, scale, quality, isJpg, isSingle);
    }

    public function SwfTexture (symbol :String, disp :DisplayObject, scale :Number, quality :String, isJpg:Boolean, isSingle:Boolean) {
        this.symbol = symbol;
