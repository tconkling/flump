...
{
	{
		// Ensure the NativeApplicationState is constructed before starting load.
		NativeApplicationState.isActive;
	}
	...
	public function load(future : FutureTask) : void
	{
		...
		if (_toLoad is String)
		{
			_zip.load(new URLRequest(String(_toLoad)));
		}
		else
		{
	...
	protected function onFileLoaded(e : FZipEvent) : void
	{
		...
		else if (name == LibraryLoader.MD5_LOCATION)
		{ // Nothing to verify
		}
		else
		{
	...
	{
		var textureGenerator : ITextureGenerator
		...
		if (_lib.textureFormat == "atf")
		{
			...
			textureGenerator.fromAtfData(function (texture : Texture) : void
			{
				baseTextureLoaded(texture, atlas);
				ByteArray(bytes).clear();
				finaliseTextureUploads(textureGenerator);
			}, bytes, scale, _libLoader.generateMipMaps);
		}
		else
		{
			...
			atlasFuture.succeeded.connect(function (img : LoadedImage) : void
			{
				...
				textureGenerator = new TextureGenerator();
				textureGenerator.fromBitmapData(function (texture : Texture) : void
				{
					baseTextureLoaded(texture, atlas);
				...
				}, img.bitmapData, _libLoader.generateMipMaps, false, scale);
			});
		}
	}
	...
	protected function finaliseTextureUploads(textureGenerator : ITextureGenerator) : void
	{
		var disposableGenerator : IDisposable = textureGenerator as IDisposable;
	...
	protected var _pngCount : uint = 0;
